...
# Manifests live at <repo>/manifests/weka/core in this repo's layout.
MANIFESTS_DIR="$REPO_ROOT/manifests/weka/core"

if [[ ! -d "$MANIFESTS_DIR" ]]; then
    echo "[ERROR] Manifests dir not found: $MANIFESTS_DIR"
    exit 1
fi
...
BACKEND_IPS=$(aws ec2 describe-instances ${REGION_FLAG[@]+"${REGION_FLAG[@]}"} \
...
WEKA_PASSWORD=$(aws secretsmanager get-secret-value ${REGION_FLAG[@]+"${REGION_FLAG[@]}"} \
