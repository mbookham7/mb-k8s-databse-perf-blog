...
# -----------------------------------------------------------------------------
# This repo only ships the weka-dedicated model; default to it so callers
# never need --module.
MODULE="${MODULE:-weka-dedicated}"

# Manifests live at <repo>/manifests/weka (not <repo>/<module>/manifests).
MANIFESTS_DIR="$REPO_ROOT/manifests/weka"
...
wait_for_status() {
    ...
    for ((i = 1; i <= max; i++)); do
        ...
        current=$(kubectl get ${ns_flag[@]+"${ns_flag[@]}"} $resource -o jsonpath="$jsonpath" 2>/dev/null || echo "")
...
do_cleanup() {
    ...
    aws eks update-kubeconfig --name "$cluster_name" ${REGION_FLAG[@]+"${REGION_FLAG[@]}"}
...
if [[ "$NEEDS_MANIFEST_GENERATION" == "true" ]] && [[ -n "$WEKA_BACKEND_NAME" && -n "$WEKA_SECRET_ARN" ]]; then
    if [[ ! -f "$MANIFESTS_DIR/core/weka-client.yaml" || ! -f "$MANIFESTS_DIR/core/csi-wekafs-api-secret.yaml" ]]; then
        ...
        "$SCRIPT_DIR/generate-manifests.sh" ${GEN_FLAGS[@]+"${GEN_FLAGS[@]}"}
        echo ""
    else
...
if [[ "$NEEDS_ENSURE_NICS" == "true" ]]; then
    ...
    # ENI creation + attachment can take several minutes; allow up to ~8 min.
    if ! wait_for_status "ensure-nics completed" \
        "wekapolicies ensure-nics-policy" "$WEKA_OPERATOR_NS" \
        '{.status.status}' "Done" "Failed" 96 5; then
