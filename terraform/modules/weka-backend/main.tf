...
module "weka_deployment" {
  ...
  # IAM (use existing roles). The upstream module auto-creates roles only when
  # these are "" (empty string); a null fails its `== ""` check, so coerce here.
  instance_iam_profile_arn = var.instance_iam_profile_arn != null ? var.instance_iam_profile_arn : ""
  lambda_iam_role_arn      = var.lambda_iam_role_arn != null ? var.lambda_iam_role_arn : ""
  sfn_iam_role_arn         = var.sfn_iam_role_arn != null ? var.sfn_iam_role_arn : ""
  event_iam_role_arn       = var.event_iam_role_arn != null ? var.event_iam_role_arn : ""
